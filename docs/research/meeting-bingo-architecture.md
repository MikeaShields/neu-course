...
graph TB
    subgraph ReactApp["React Application"]
        UI[UI Layer] --> Context[Context State]
        Game[Game Logic] --> Context
    ...
    Speech -.-> WebSpeech
    ReactApp -->|HTTPS| Vercel[Vercel Edge Network]
```
...
```mermaid
flowchart LR
    A["Select Category"] --> B["Shuffle Words"]
    B --> C["Pick 24 Words"]
    C --> D["Render Card"]
```
...
```mermaid
graph LR
    MP[Multiplayer] --> A[WebSocket Server]
    MP --> B["Peer-to-Peer WebRTC"]
    MP --> C[Firebase Realtime DB]

    A --> A1["Real-time sync"]
    A --> A2[Shared game state]
    A --> A3["Cost: $5-20/month"]

    B --> B1[No backend required]
    B --> B2[Direct connections]
    B --> B3["Cost: Free"]

    C --> C1[Managed sync]
    C --> C2[Easy implementation]
    C --> C3["Cost: Free up to 100"]
```
